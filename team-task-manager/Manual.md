...
-   **端對端測試 (End-to-End Testing)**:
    ...
    -   **主要測試案例**: 
        -   **使用者認證流程**: 測試涵蓋了完整的註冊與登入流程，確保系統的健全性。
            1.  **導航至註冊頁面**: 測試腳本首先會訪問應用程式首頁，並點擊「註冊」按鈕。
            2.  **填寫註冊表單**: 在 `/register` 頁面，腳本會自動填入預設的測試使用者名稱、信箱和密碼。
            3.  **提交與驗證**: 點擊「註冊」按鈕後，腳本會等待後端的回應。
            4.  **確認註冊成功**: 系統應顯示「註冊成功」的提示訊息。腳本會驗證此訊息是否存在。
            5.  **返回首頁**: 點擊成功提示中的「確認」按鈕後，應用程式應自動導航回應用程式首頁。
            6.  **執行登入**: 腳本會點擊「登入」按鈕，並使用剛剛註冊的帳號密碼進行登入。
            7.  **驗證登入結果**: 成功登入後，應用程式應跳轉至任務看板頁面 (`/flow.html`)。腳本會驗證當前頁面的 URL 是否正確，並檢查頁面上是否出現使用者名稱，以確認登入狀態。

    #### ⛔ 最新 e2e 測試結果（2025-08-11）
    - 測試腳本失敗於「註冊成功」畫面：
        - 預期顯示 `Registration Successful` 訊息與 `OK` 按鈕，但畫面未出現該訊息。
        - Playwright 報錯：`Timed out 5000ms waiting for expect(locator).toBeVisible() Locator: getByText('Registration Successful')`。
    - 請檢查 RegisterSuccessPage 是否正確渲染英文訊息與 OK 按鈕，並確保註冊流程導向該頁。
