...
2.  **註冊/登入**:
    -   **註冊**: 使用頁面右上角的「註冊」按鈕建立新帳戶。成功後，系統將自動為您登入並導向任務看板。
    -   **登入**: 如果您已有帳戶，請點擊「登入」。
3.  **進入任務看板**: 成功登入後，您將被導向到 `/flow.html` 的主任務看板。
4.  **管理任務**: 您可以建立、編輯、刪除任務，並使用篩選器來尋找特定任務。
5.  **登出**: 點擊右上角的使用者頭像，在下拉選單中選擇「登出」。
6.  **使用 Chatbot**: 在右下角點擊聊天圖示，即可與 AI 助理互動。
...
|   |-- database.sql    # 資料庫初始化腳本 (包含 users 和 tasks 表格)
|   |-- index.js        # API 伺服器進入點
...
|   |-- i18n/           # 國際化 (i18next) 設定
|   |-- pages/          # 頁面級元件 (對應一個路由)
|   |-- stores/         # 全域狀態管理 (Zustand)，負責處理 API 請求與前端狀態
...
-   **端對端測試 (End-to-End Testing)**:
    ...
    -   **主要測試案例**: 
        -   **使用者認證流程**: 測試涵蓋了完整的註冊、自動登入、登出流程，確保系統的健全性。
            1.  **導航至註冊頁面**: 測試腳本首先會訪問應用程式首頁，並點擊「註冊」按鈕。
            2.  **填寫註冊表單**: 在 `/register` 頁面，腳本會自動填入一個隨機產生的新使用者名稱、信箱和密碼。
            3.  **提交與驗證**: 點擊「註冊」按鈕後，腳本會驗證應用程式是否自動導向到任務看板頁面 (`/flow.html`)，表示註冊並自動登入成功。
            4.  **驗證登入狀態**: 腳本會檢查頁面上是否出現使用者頭像，以確認登入狀態。
            5.  **執行登出**: 點擊使用者頭像，然後在下拉選單中點擊「登出」。
            6.  **驗證登出結果**: 成功登出後，應用程式應跳轉回首頁 (`/`)。腳本會驗證當前 URL 是否正確。
...
當您使用 `npm run dev:docker` 啟動服務後，可以透過以下方式直接與 Docker 內的 PostgreSQL 資料庫互動：

1.  **找到資料庫容器名稱**:
    -   首先，打開一個新的終端機視窗，執行以下指令來列出正在運行的容器：
        ```bash
        docker ps
        ```
    -   在列表中找到 `IMAGE` 為 `postgres` 的容器，並複製其 `NAMES` (例如 `team-task-manager-db-1`)。

2.  **進入容器並連接資料庫**:
    -   使用以下指令進入容器的 shell 環境：
        ```bash
        docker exec -it <容器名稱> bash
        ```
        (請將 `<容器名稱>` 替換為您上一步複製的名稱)。
    -   成功進入後，使用 `psql` 工具連接到 `task_manager` 資料庫。密碼為 `postgres` (這是 `docker-compose.yml` 中設定的預設密碼)。
        ```bash
        psql -U postgres -d task_manager
        ```

3.  **執行 SQL 查詢**:
    -   成功連接後，您就可以執行標準的 SQL 指令來查詢或操作資料。
    -   **查詢所有使用者**:
        ```sql
        SELECT * FROM users;
        ```
    -   **查詢所有任務**:
        ...
        SELECT * FROM tasks;
        ```
    -   **退出 psql**: 輸入 `\q` 並按下 Enter。
    -   **退出容器**: 輸入 `exit` 並按下 Enter。
